...
import { FaPaypal, FaHeart } from "react-icons/fa";
import { createDonationAPI } from "../axios/axios.api";
...
const DonatePage = () => {
    ...
    const handleDonate = async () => {
        try {
            setLoading(true);
            const response = await createDonationAPI(amount, message);
            if (response?.approvalLink) {
                window.location.href = response.approvalLink;
            } else {
                alert("Failed to get approval link.");
            }
        } catch (error) {
    ...
    return (
        <>
            {/* Wrapper có background gradient và các phần trang trí */}
            <div className="relative bg-gradient-to-b from-blue-50 to-white overflow-hidden">
                {/* Phần trang trí: hình tròn màu */}
                <div className="absolute top-0 left-0 w-40 h-40 bg-pink-300 rounded-full mix-blend-multiply filter blur-2xl opacity-70 animate-pulse"></div>
                <div className="absolute bottom-0 right-0 w-40 h-40 bg-purple-300 rounded-full mix-blend-multiply filter blur-2xl opacity-70 animate-pulse"></div>

                <MobileTopBar />
                <div className="px-12 md:px-8 lg:px-8 py-10 relative">
                    <div className="flex flex-col font-lora text-gray-700 md:flex-row items-center justify-center gap-10 md:gap-20">
                        ...
                        <div className="text-justify space-y-6 md:text-left">
                            <h1 className="text-2xl indent-3 md:text-3xl font-bold text-gray-800">
                                Donate to Save a Life <FaHeart className="inline text-red-500" /> 🐾
                            </h1>
                    ...
                    <div className="p-6 bg-gray-100 rounded-lg max-w-md mx-auto shadow-lg">
                    ...
                    <div className="mt-6 text-center px-6">
                        <button
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg text-lg w-full md:w-auto inline-flex items-center justify-center gap-2 shadow-lg transition-all duration-300 hover:scale-105"
                            onClick={handleDonate}
                        >
                            <FaPaypal className="text-2xl" /> Donate via PayPal
                        </button>
                    </div>
                </div>
            </div>
